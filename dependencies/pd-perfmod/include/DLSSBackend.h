...
/**
 * DLSS Backend implementation (priority=0).
 * Loads NVIDIA Streamline DLSS at runtime.
 * For now, provides stub implementations until vendor SDK is integrated.
 */
class DLSSBackend : public IUpscalerBackend {
public:
    DLSSBackend();
    virtual ~DLSSBackend();
    ...
    const char* get_name() const override { return "DLSS"; }
    int get_method_id() const override { return 0; }  // DLSS = 0
...
private:
    ...
    // Helper to check if DLSS DLL is available
    bool probe_dlss_dll();
};
